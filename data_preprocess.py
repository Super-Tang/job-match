...
for line in job:
    if line["岗位编号"] not in job_info:
        job_info[line["岗位编号"]] = []
    job_info[line["岗位编号"]].append([line['岗位描述'], line["排序"]])
...
for key in job_info:
    ...
    for item in job_info[key]:
        ...
        for personnel_candidate in personnel_candidates:
            ...
            negative_samples = random.sample(all_samples, 10)
            data.extend([{'job_description': job_description, 'item1': info, 'item2': negative_sample, 'label': 1} for
                         negative_sample in negative_samples[:5] if negative_sample != info])
            positive += 10
            data.extend([{'job_description': job_description, 'item2': info, 'item1': negative_sample, 'label': 0} for
                         negative_sample in negative_samples[5:] if negative_sample != info])
            negative_samples1 = random.sample(all_samples, 20)
            negative_samples2 = random.sample(all_samples, 20)
...
out = open('data.json', 'w', encoding='utf-8')
