...
def predict(model, tokenizer, task, candidates, max_num=5):
    ...
    return [(candidates[item[0]], item[1]) for item in scores[:5]]
...
def test(model, tokenizer, file_path):
    ...
    for key in data.keys():
        candidates = data[key][0]
        filter_candidate = predict(model, tokenizer, key, candidates)
        filter_data[key] = []
        filter_data_all[key] = [[candidate[0] for candidate in filter_candidate], data[key][1]]
        for item in filter_candidate:
            info = f"姓名：{item[0]['姓名']}，职位：{item[0]['职位']}，工作科室：{item[0]['工作部门']}，专业技能：{item[0]['专业技能']}，得分：{item[1]}，岗位编号：[{item[0]['岗位编号']},{data[key][1]}]"
