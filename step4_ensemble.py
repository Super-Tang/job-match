...
def ensemble(result, max_count=5):
    ...
    for i, key in enumerate(result.keys()):
        ...
        for item in scores.keys():
            if item not in ret:
                ret[item] = 0
            elif i == 0: ret[item] = scores[item]
            if i > 0:
                ret[item] += scores[item] /(1+i)
            if len(all_result) < 5: all_result['paraphrase-' + str(i)].append((item, scores[item]))
    for item in ret:
        ret[item] = math.exp(ret[item]) # * cal_score(item)
